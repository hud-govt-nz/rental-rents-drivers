...
combined_nz <- combined_nz %>%
  ...
  mutate(ppd.change = (log(ppd) - log(lag(ppd,t)))*100) %>%
  mutate(ppd_hud.demeaned = ppd_hud - mean(ppd_hud,na.rm = T)) %>%
  mutate(ppd_hud.change = ppd_hud - lag(ppd_hud,t)) %>%
  # Deviation from sample mean
  mutate(earnings.change.demeaned = earnings.change - mean(earnings.change,na.rm = T)) %>%
  # Deviation from last 4 quarters mean
  #mutate(earnings.change.demeaned = earnings.change - (lag(earnings.change,3) + lag(earnings.change,6)+ lag(earnings.change,9)+ lag(earnings.change,12))/4) %>%
...
ggplot(combined_nz_quarterly, aes(x = date, y = rpi.change)) +
  geom_line(col = col[2], size = 2, alpha = 0.3) +
  geom_smooth(se = F, span = 0.1, col = col[2], alpha = 1, size = 2) +
  geom_line(data = combined_nz_quarterly, aes(x = date, y = vacant_time.change), col = col[3], size = 2, alpha = 0.3) +
  
  theme1()+
  labs(title = "Annual rent inflation (%)",
       y ="",
       x = "",
       col = "",
       alpha ="") +
...
plot_rpi <- ggplot(combined_nz_quarterly, aes(x = date, y = rpi.change)) +
  geom_line(col = col[1], size = 2, alpha = 0.1) +
  geom_smooth(se = F, span = 0.1, col = col[1], alpha = 1, size = 2) +
  theme1()+
...
png("results/rpi.png", width = 1500, height = 1200, res=120)
plot_rpi
dev.off()
...
Acf(rpi_ts, lag.max = 10)

Pacf(rpi_ts, lag.max = 10, plot = TRUE, na.action = na.contiguous)
...
model1b <- lm(rpi.change ~ 
            ...
            #+lag(rpi.change,i+3)  
                ...
                +lag(earnings.change.demeaned,i)
             # +lag(earnings.change.demeaned,i+1)
             # +lag(earnings.change.demeaned,i+2)
...
# Add change in floating mortgage rates
...
plot_data <- data.frame(predicted = predict(model1e), 
                       observed = combined_nz_quarterly$rpi.change[c(8:56, 60:66)],
                       date = combined_nz_quarterly$date[c(8:56, 60:66)]) %>%
...
png("results/fitted_rpi.png", width = 1500, height = 1200, res=120)
plot_fitted
dev.off()
